        ...
        <li>
            <div class="dropdown-menu dropdown-menu-head pull-right">
				...
				<div <?php if($Num>5){echo "style='width:300px;height:400px;overflow:auto;'";} ?> > 
	                    ...
	                    <li class="new">
	                    	<a href="../imitation_2/dateworks.php">
	                    		<span class="desc"><span >进入日程管理</span></span>
	                    	</a>
	                    </li>
	                </ul>
                </div>
            ...
            <div class="dropdown-menu dropdown-menu-head pull-right">
				...
				<div <?php if($Num3>5){echo "style='width:300px;height:400px;overflow:auto;'";} ?> > 
					<ul class="dropdown-list normal-list">
	                    ...
	                    <li class="new">
	                    	<a href="../../remindT.php">
	                    		<span class="desc"><span >进入提醒页面</span></span>
	                        </a>
	                    </li>
               	 	</ul>
                </div>
        ...
        <li>
            <div class="dropdown-menu dropdown-menu-head pull-right">
                <h5 class="title">缴费通知</h5>
            		<?php
            			$sqlC = "SELECT id as Num  from  专案待缴费  where (费用状态=0 or 费用状态=1 or 费用状态=8) and DATEDIFF(缴费期限,DATE_FORMAT(NOW(),'%Y-%m-%d'))<'10'";
						$resultC = $conn->query($sqlC);
						$tmp_arr = array();
						if($resultC->num_rows>0){
							while($rowC = $resultC->fetch_assoc()){
//								$Num4 = $rowC['Num'];
				...
				<div <?php if($Num4>5){echo "style='width:300px;height:400px;overflow:auto;'";} ?> > 
					<ul class="dropdown-list normal-list">
						<?php
							$sql4 = "SELECT id,`案卷号`,类型,`申请号`,费用名称,`金额`,`缴费期限`,申请人,案源人,代理人,申请日,`专利名称`,提醒时间,DATEDIFF(缴费期限,DATE_FORMAT(NOW(),'%Y-%m-%d')) AS 计算天数,申请人id  from  专案待缴费  where (费用状态=0 or 费用状态=1 or 费用状态=8) and DATEDIFF(缴费期限,DATE_FORMAT(NOW(),'%Y-%m-%d'))<'10'";
						...
						<li class="new">
                            ...
                            <em class="small">(<?php echo $row4['计算天数'];?>)</em>
	                    </li>
						<?php
								}
							}
