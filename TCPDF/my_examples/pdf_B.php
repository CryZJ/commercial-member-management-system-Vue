 ...
 * 创建时间：2018年2月2日
 * 创建者：KingShen
 * 说明：用于导出数据库中“转让/转移申请/注册商标申请书”和等图片信息，输出为PDF文件于网页上浏览、下载、打印，使用了TCPDF类库
 * */
...
//$ajh = "03025bO4bO";
 ...
 $data_one_arr = ""; 
 $sql = "SELECT a.委托书id,a.申请人id,a.申请人商标地址,a.注册号,b.转让人id,b.转让人商标地址,b.外国受让人的国内接收人,b.邮政编码,b.国内接收人地址,b.共有商标是,b.共有商标否 FROM 商标_案件 a,商标_案件附加信息 b,商标_委托书 c WHERE a.案卷号='".$ajh."' AND a.案卷号=b.案卷号 AND a.委托书id=c.id";		
 ...
 if($result->num_rows>0){
 	while($row = $result->fetch_assoc()){
		...
		$data_one_arr["转让人id"] = $row["转让人id"];
		$data_one_arr['转让人地址中'] = $row['转让人商标地址'];
		$data_one_arr["外国受让人的国内接收人"] = $row["外国受让人的国内接收人"];
		$data_one_arr["邮政编码"] = $row["邮政编码"];
		$data_one_arr["国内接收人地址"] = $row["国内接收人地址"];
...
//				$data_one_arr['受让人地址中'] = $row['地址'];
	...
	if(!empty($data_one_arr["转让人id"])){
		$sql = "SELECT 申请人,英文名,地址,地址E FROM 申请人 WHERE id='".$data_one_arr["转让人id"]."'";
		$result = $conn->query($sql);
		if($result->num_rows>0){
	 		while($row = $result->fetch_assoc()){
	 			$data_one_arr['转让人名称中'] = $row['申请人'];
				$data_one_arr['转让人名称英'] = $row['英文名'];
//				$data_one_arr['转让人地址中'] = $row['地址'];
				$data_one_arr['转让人地址英'] = $row['地址E'];
	 		}
		}
	}
 }
...
<h1 style="text-align:center">转让/转移申请/注册商标申请书</h1>
<table style="border-collapse:collapse;width:925px;"  cellspacing="3" cellpadding="2" >
	<tr>
		<td align="right" style="width:25%;">转让人名称(中文)：</td>
		<td align="left">'.$data_one_arr['转让人名称中'].'</td>
	</tr>
	<tr>
		<td align="right" style="width:25%;">(英文)：</td>
		<td align="left">'.$data_one_arr['转让人名称英'].'</td>
	</tr>
	<tr>
		<td align="right" style="width:25%;">转让人地址(中文)：</td>
		<td align="left">'.$data_one_arr['转让人地址中'].'</td>
	</tr>
	<tr>
		<td align="right" style="width:25%;">(英文)：</td>
		<td align="left">'.$data_one_arr['转让人地址英'].'</td>
	</tr>
	<tr>
		<td align="right" style="width:25%;">受让人名称(中文)：</td>
	...
	<tr>
		<td align="right" style="width:25%;">受让人地址(中文)：</td>
	...
	<tr>
		<td align="right" style="width:25%;">外国受让人的国内接收人：</td>
		<td align="left">'.$data_one_arr["外国受让人的国内接收人"].'</td>
	</tr>
	<tr>
		<td align="right" style="width:25%;">国内接收人地址：</td>
		<td align="left">'.$data_one_arr["国内接收人地址"].'</td>
	</tr>
	<tr>
		<td align="right" style="width:25%;">邮政编码：</td>
		<td align="left">'.$data_one_arr["邮政编码"].'</td>
	</tr>
	<tr>
		<td align="right" style="width:25%;">商标代理机构名称：</td>
...
<table style="border-collapse:collapse;width:925px;"  cellspacing="3" cellpadding="2" >
	<tr>
		<td style="width:25%;">转让人章戳(签名)</td>
		<td style="width:25%;">受让人章戳(签名)</td>
		<td style="width:25%;">代理机构章戳</td>
	</tr>
</table>
';
