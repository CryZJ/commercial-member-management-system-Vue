...
$data_one_arr = ""; 
 $sql = "SELECT a.委托书id,a.申请人id,a.申请人商标地址,a.注册号,a.类别,b.共有商标是,b.共有商标否 FROM 商标_案件 a,商标_案件附加信息 b,商标_委托书 c WHERE a.案卷号='".$ajh."' AND a.案卷号=b.案卷号 AND a.委托书id=c.id";		
 ...
 if($result->num_rows>0){
	...
	if(!empty($data_one_arr["申请人id"])){
		$sql = "SELECT 申请人,英文名,地址,地址E,证件号 FROM 申请人 WHERE id='".$data_one_arr["申请人id"]."'";
		$result = $conn->query($sql);
		if($result->num_rows>0){
	 		while($row = $result->fetch_assoc()){
...
<h1 style="text-align:center">商标续展注册申请书</h1>
<table style="border-collapse:collapse;width:925px;"  cellspacing="3" cellpadding="2" >
	...
	<tr>
		<td align="right" style="width:25%;">证件号：</td>
		<td align="left">'.$data_one_arr['受让人证件号'].'</td>
	</tr>
	<tr>
	...
	<tr>
		<td align="right" style="width:25%;">类别：</td>
		<td align="left">'.$data_one_arr["类别"].'</td>
	</tr>
</table>
<div style="height:350;" ></div><div style="height:350;" ></div><div style="height:350;" ></div>
<div style="height:350;" ></div><div style="height:350;" ></div>
<table style="border-collapse:collapse;width:925px;"  cellspacing="3" cellpadding="2" >
	<tr>
...
$file_namepdf = $ajh."_续展申请书.pdf";
//$pdf->Output($file_namepdf, 'I');
