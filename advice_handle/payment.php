    	...
    	<header class="panel-heading custom-tab">
			<span class="tools pull-right">
	      	...
	      	<ul class="nav nav-tabs">
		        ...
		        <li class="active"><a href="#about-3" data-toggle="tab"><i class="fa fa-user"></i>缴费通知文件</a></li>
       	...
       	<div class="panel-body">
    		...
    		<div class="tab-content">
                ...
                <section id="unseen">
                	<header class="panel-heading">
						<p>本次上传的缴费通知文件有<em id="filenum_jf" style="font-size: 25px; color: #0075B0;"></em>个</p>
					</header>	
						<table class="display table table-bordered table-striped" style="font-size: 5px;" >
					        ...
					        <?php
					        	...
					        	require("../conn.php");
//										$sql = "SELECT id,上传时间,文件名称,通知书名称,通知书编码,专利名称,案卷号,原案卷号,申请号,申请日,发文日,申请号是否一样 FROM 临时文件 WHERE 上传情况='0' AND 通知书编码='200701' AND 案件存在='0' ORDER BY 上传时间 DESC ";
										$sql = "SELECT a.id,上传时间,a.文件名称,a.通知书名称,a.通知书编码,a.专利名称,a.案卷号,a.原案卷号,a.申请号,a.申请日,a.发文日,a.申请号是否一样,c.状态,c.冻结状态 FROM 临时文件 a,(SELECT 案卷号,状态,冻结状态,案件分类 FROM (SELECT 案卷号,状态,冻结状态,'专利信息' AS 案件分类 FROM 专利信息 UNION SELECT 案卷号,状态,冻结状态,'专案_年费' AS 案件分类 FROM 专案_年费 UNION SELECT 案卷号,状态,冻结状态,'专案_复审等' AS 案件分类 FROM 专案_复审等) AS c) c  WHERE a.案卷号=c.案卷号 AND a.上传情况='0' AND a.通知书编码='200701' AND a.案件存在='0' ORDER BY a.案卷号;";
										$result = $conn->query($sql);
										$i=0;
										$pay_id='';
										if($result->num_rows >0){
											while($row = $result->fetch_assoc()){
												$pay_id = $pay_id.$row['id'].",";
									...
									?>
										<tr>
											...
											<td>
												...
												<button style="font-size:15px;padding: 0;" class="btn btn-default" id="<?php echo $row['id'];?>" onclick="pay_one(this.id)" >确认</button>
									...
									<?php			
										...
										$pay_id = substr($pay_id, 0,strlen($pay_id)-1);
										$conn->close();
						      ?>	
					        </tbody>
	 					</table>
 					  	<div align="center">
                  			<button id='sure_3' onclick="pay('<?php echo $pay_id; ?>')">一键确认</button>
                  		</div>
                </section>
           		</div>  
           		<!--缴费通知 end-->	   
