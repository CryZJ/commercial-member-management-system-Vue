        ...
        <section class="panel">
            <header class="panel-heading">
            	<form action="#" method="post" enctype="multipart/form-data">
	            	<h3 align="center">通知书批量导入及信息确认</h3>
...
//											echo $ext;
												if($ext=="zip"){
													//获取xml信息
													$returndata =  acceptor_readxml_all($files[$i]['tmp_name']);
													if($returndata['结果']=="成功"){
														//编辑路径并保存
	...
	//													echo $upload_path."<br/>";
														...
														$now_time = date("Y-m-d");
														//更新“专利信息”
														$sql = "update 专利信息  set 申请号='".$returndata['申请号']."',申请日='".$returndata['申请日']."',复审费减比例='".$returndata['复审费减比例']."',年费费减比例='".$returndata['年费费减比例']."',状态='申请中' where 案卷号='$ajh'";
	//													echo $sql."<br/>";
														$result = $conn->query($sql);
														//更新“案卷流程及文档”
														$sql2 = "insert into 案卷流程及文档(案卷号,流程,处理人,时间,文件路径,通知书名称) value(";
														$sql2 .= " '$ajh','受理导入','$name','$now_time','$upload_path','".$returndata['通知书名称']."')";
	//													echo $sql2;
...
//												echo $base_data[$i]['申请号']."/".substr($base_data[$i]['申请号'],4,1)."<br/>";
												if(substr($base_data[$i]['申请号'],4,1)==1){
									...
									?>
													<tr>
														...
														<td><?php echo $base_data[$i]['年费费减比例']; ?></td>
														<td><?php echo $key; ?></td>
														<td><?php echo $value; ?></td>
														<td><input type="date"  value="<?php echo date('Y-m-d',strtotime('-1months',strtotime($base_data[$i]['缴费截止日期']))); ?>"/> </td>
														<td><?php $now_t=date("Y-m-d"); echo diffBetweenTwoDays($now_t,$base_data[$i]['缴费截止日期']); ?></td>
														<td><?php echo $base_data[$i]['缴费截止日期']; ?></td>
													</tr>
									<?php					
													}
									?>
