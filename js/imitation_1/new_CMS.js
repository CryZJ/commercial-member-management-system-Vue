...
//      alert(CaseBz);
        ...
        if(document.getElementById("ClientName").value){
        	//装载数据
        	fd_file.append("falg","CMS_Save");
        	fd_file.append("ayr",ayr);
        	fd_file.append("TabMes_1",TabMes_1);
        	fd_file.append("TabMes_2",TabMes_2);
        	fd_file.append("TabMes_3",TabMes_3);
        	fd_file.append("TabMes_4",TabMes_4);
        	fd_file.append("TabMes_5",TabMes_5);
        	fd_file.append("TabMes_6",TabMes_6);
        	fd_file.append("TabMes_7",TabMes_7);
        	fd_file.append("CaseBz",CaseBz);
...
//                  console.log(data);
                    setTimeout(function(){
						if(data == 'ok'){
	                        if(confirm("企业资料新建成功,是否继续新建企业信息")){
			...
			if(winobj.closed){
				...
				if(typeof(Storage)!=="undefined"){
					if(localStorage.ayr_name){
						document.getElementById("ayr").value = localStorage.ayr_name;
