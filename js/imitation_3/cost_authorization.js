...
function SelectRefreshTable(tab_idstr){
	switch(tab_idstr){
		...
		case "dynamic-table_3":
			...
			break;
		case "dynamic-table_4":
			Refresh_DynmicTable_4();
			break;
		case "dynamic-table_5":
			Refresh_DynmicTable_5();
			break;
		case "dynamic-table_6":
			Refresh_DynmicTable_6();
			break;
		default :
			alert("服务器错误");
	}
...
//			MesId = MesId+'|'+tab.rows[i].cells[3].innerHTML;
			MesId += ','+tab.rows[i].cells[0].getElementsByTagName('input')[0].id;
		}
	}
	if(MesId){
		MesId = MesId.substr(1);
//		console.log(MesId);
...
//				console.log(data);
				if(data.state){
					if(typeof(Storage) !== "undefined"){
						...
						openWin("authorization_notice_parent.php","_blank","");
					}else{
...
function Cost_alter(a_doc){
	...
	cost_value.value = tr_doc.cells[5].innerHTML;
