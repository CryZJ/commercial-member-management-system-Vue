...
if($str_id != ""){
	...
	foreach($arr_id as $ky => $id){
		$sql_s="select id,b.案卷号,b.案源人,b.代理人,类型,专利名称,冻结状态,b.状态,申请号,申请人,申请人id  from `专案_复审等` b where b.冻结状态<>'3' and b.冻结状态<>'2' and 状态<>9 and id='".$id."' group by  b.`案卷号`";
		$result = $conn->query($sql_s);
		if($result->num_rows>0){
			while($row = $result->fetch_assoc()){
				$sql = "SELECT 申请人 FROM 申请人 WHERE FIND_IN_SET(id,'".$row['申请人id']."')";
				...
				$objPHPExcel->getActiveSheet()->setCellValue('E'.$i,$sqr_str);
