...
$objPHPExcel->getActiveSheet()->setCellValue('B1','支出项目');
$objPHPExcel->getActiveSheet()->setCellValue('C1','金额');
$objPHPExcel->getActiveSheet()->setCellValue('D1','支出日期');
$objPHPExcel->getActiveSheet()->setCellValue('E1','收款人');
$objPHPExcel->getActiveSheet()->setCellValue('F1','付款人');
//居中
...
//$sql_s = "SELECT id,支出项目,金额,支出日期,收款人,付款人 FROM 支出记录 WHERE  FIND_IN_SET(id,'".$str_id."') ORDER BY 支出日期 ";
$sql_s = "SELECT id,支出项目,金额,支出日期,收款人,付款人 FROM ".$expend_record." WHERE  FIND_IN_SET(id,'".$str_id."') ORDER BY 支出日期 ";
$result = $conn->query($sql_s);
$i=2;
if($result->num_rows>0){
	while($row = $result->fetch_assoc()){
		$objPHPExcel->getActiveSheet()->setCellValue('A'.$i,$i-1);
		$objPHPExcel->getActiveSheet()->setCellValue('B'.$i,$row['支出项目']);
		$objPHPExcel->getActiveSheet()->setCellValue('C'.$i,$row['金额']);
//		$objPHPExcel->getActiveSheet()->setCellValueExplicit('D'.$i,$row['规费'],PHPExcel_Cell_DataType::TYPE_STRING);//显示字符串
		$objPHPExcel->getActiveSheet()->setCellValue('D'.$i,$row['支出日期']);
		$objPHPExcel->getActiveSheet()->setCellValue('E'.$i,$row['收款人']);
		$objPHPExcel->getActiveSheet()->setCellValue('F'.$i,$row['付款人']);
		//日期格式化
