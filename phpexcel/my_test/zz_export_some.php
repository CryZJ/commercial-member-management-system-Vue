...
$objPHPExcel->getActiveSheet()->setCellValue('C1','著作名称');
$objPHPExcel->getActiveSheet()->setCellValue('D1','申请人');
$objPHPExcel->getActiveSheet()->setCellValue('E1','申请号');
$objPHPExcel->getActiveSheet()->setCellValue('F1','申请日期');
...
//$sql_s = "SELECT 案卷号,软件名称,申请人id,申请号,申请日期,案源人,代理人,案件备注 FROM 著作_信息 WHERE 状态<>3";
$sql_s = "SELECT 案卷号,著作名称,申请人id,申请号,申请日期,案源人,代理人,案件备注 FROM 著作_信息 WHERE FIND_IN_SET(案卷号,'".$str_ajh."') AND 状态<>3 AND 状态<>9";
...
if($result->num_rows>0){
	while($row = $result->fetch_assoc()){
		...
		$objPHPExcel->getActiveSheet()->setCellValue('C'.$i,$row['著作名称']);
		...
		$objPHPExcel->getActiveSheet()->setCellValue('F'.$i,$row['申请日期']);
