	        ...
	        <div class="tab-pane active" id="about-1">
	    		  <section id="unseen">
	    		  	<?php
	        			...
	        			//获取案卷基本情况
	        			require('../../../conn.php');
	        			$sql = "select 案件类型,id,委托人类型,案源人,代理人,注册日,注册号,委托书id,专权期始,专权期末,备注,申请人id,申请人商标地址,状态,类别,商标说明,商品服务  from 商标_案件 a  where a.案卷号='".$ajh."'";
		        	...
		        	<?php 
	                	...
	                	<br /><br />
	                	<?php
		                	$sqlZ = "select 共有商标是,邮政编码,国内接收人地址,转让人,转让人类型,转让人id,外国受让人的国内接收人,转让人委托书id from 商标_案件附加信息 where 案卷号 = '".$ajh."'";
		                		$resultZ=$conn->query($sqlZ);
		                		if($resultZ -> num_rows>0){
		                			while($rowZ = $resultZ->fetch_assoc()){
		                				$cReF2  =$rowZ['转让人委托书id'];
										$WGJSR = $rowZ['外国受让人的国内接收人'];
		                				$FAPAIC  =$rowZ['国内接收人地址'];
		                				$Stamp  =$rowZ['邮政编码'];
		                				$APeoA  =$rowZ['转让人'];
		                				$APeoType  =$rowZ['转让人类型'];
	                		...
	                		?>
	                		<p style="color: #B6B6B6;">如果修改信息按Enter键没有提示已保存请按右上的保存按钮</p>
		        	<table class="table table-striped table-bordered" id="tabUserInfo_1">
			                <tr>
				        		<th hidden="hidden">受让人类型/转让人类型</th>
				        		<td hidden="hidden"><?php echo $ctype.'/'.$APeoType; ?></td>
				        		<th>注册号</th>
				        		<td colspan="2">
				        			<?php
				        				if(strlen($cMKN)==0){
...
//	                			}
//	                		}
	                		$Csqrc=$Csqre=$Caddc=$Cadde=$Cidn=$Cenum=$Ccode=$Ccoty='';
	                		$sql = "SELECT 委托人id,委托人地址 FROM 商标_委托书 WHERE id='".$cReF2."'";
							$result = $conn->query($sql);
							if($result->num_rows>0){
								while($row = $result->fetch_assoc()){
									$wtr_id = $row['委托人id'];
	                		...
	                		$sql3 = "select * from 申请人 where id = '".$wtr_id."'";
	                		$result3 = $conn->query($sql3);
	                		if($result3->num_rows>0){
	                			while($row3 = $result3->fetch_assoc()){
	                				$Csqrc = $row3['申请人'];
...
//	                		echo $Csqrc.','.$Csqre.','.$Caddc.','.$Cadde.','.$Cidn.','.$Cenum.','.$Ccode.','.$Ccoty;
	                	?>
				       		<tr>
				        		<th>转让人(中文名)</th>
				        		<td colspan="2"><?php echo $Csqrc; ?></td>
								<th>转让人(英文名)</th>
				        		<td colspan="2"><?php echo $Csqre; ?></td>
				       		</tr>
				       		<tr>
				       			<th>证件号</th>
								<td><?php echo $Cidn; ?></td>
	                ...
	                <table class="table table-striped table-bordered" id="tabUserInfo_2">
	                	<tr>
	                		<th>外国受让人的国内接收人</th>
							<td><input type="text" class="Change" style="width: 80%;" value="<?php echo $WGJSR; ?>" onchange="changeMes2('WGJSR',this)" /></td>
							<th>邮政编码</th>
							<td><input type="text" class="Change" style="width: 80%;" value="<?php echo $Stamp; ?>" onchange="changeMes2('YZBM',this)" /></td>
							<th>是否有商标</th>
							<td style="width: 500px;">
								<select onchange="changeMes2('SFYSB',this)">
									<?php
										if($BLogo){
									?>
	                	...
	                	<tr>
	                		<th colspan="1" >国内接收人地址</th>
	                		<td colspan="5" ><input type="text" class="Change" style="width: 80%;" value="<?php echo $FAPAIC; ?>" onchange="changeMes2('GNJSR',this)" /></td>
	                	</tr>
	                </table>
	                <label>案件备注：</label>
