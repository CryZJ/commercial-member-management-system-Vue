  ...
  <div class="col-sm-12">
    <section class="panel">
			...
			<?php
    			...
    			if($result->num_rows > 0){
    				while($row = $result->fetch_assoc()){
    					...
    					$firstyearnum = $row["年费首年度"];
					}
    			}else{
    				 echo "<script type=\"text/javascript\">alert('该案卷号不存在！或出错！');</script>"; 
				}
        	?>
        ...
        <div class="tab-content">                                
            ...
            <table class="table table-bordered table-striped table-condensed" >
                ...
                <tr>
                    ...
                    <th class="numeric" onclick="OpenResetAnnualFee('<?php echo $ajh; ?>','zlxx')">申请日</th>
                    <td class="numeric" onclick="OpenResetAnnualFee('<?php echo $ajh; ?>','zlxx')"><?php echo date("Y-m-d", strtotime($zlxx[1])); ?></td>
	                <th class="numeric" onclick="OpenResetAnnualFee('<?php echo $ajh; ?>','zlxx')">费减比</th>
                    <td class="numeric" onclick="OpenResetAnnualFee('<?php echo $ajh; ?>','zlxx')"><?php echo $FareCount; ?></td>
                    <th class="numeric" onclick="OpenResetAnnualFee('<?php echo $ajh; ?>','zlxx')">年费首年度</th>
                    <td class="numeric" onclick="OpenResetAnnualFee('<?php echo $ajh; ?>','zlxx')"><?php echo $firstyearnum; ?></td>
                    <th class="numeric">授权公告日</th>
	                <td class="numeric">
	                	<?php echo $zlxx[6]; ?>
              			<!--<input style="height: 26px;" name="change" type="date" id="sqgg" value="<?php echo $zlxx[6]; ?>" <?php if(($name==$zlxx[3] || $name==$zlxx[7] || $admin ==1 || $lcczy==1)&&($CaseStu==0)){ ?> onchange="MesChange('sqgg')"<?php }else{ ?>  readonly="readonly" <?php } ?>/>-->
	                </td>
                </tr>
                </thead>
                <tbody>
                <tr>
                    <!--<?php if(($name==$zlxx[3] || $name==$zlxx[7] || $admin ==1 || $lcczy==1)&&($CaseStu==0)){ ?> onchange="select_sqDate()"<?php }else{ ?>  readonly="readonly" <?php } ?>-->
	                <th class="numeric">专利名称</th>
		            <td class="numeric" colspan="5"><input style="width: 90%;" name="change" id="zlmc" value="<?php echo $zlxx[5]; ?>" <?php if(($name==$zlxx[3] || $name==$zlxx[7] || $admin ==1 || $lcczy==1)&&($CaseStu==0)){ ?> onchange="MesChange('zlmc')"<?php }else{ ?>  readonly="readonly" <?php } ?> /></td>
	                <th class="numeric">案源人</td>
                    <td class="numeric" colspan="2"><input style="width: 90%;" id="select_ayr" readonly="readonly" value="<?php echo $zlxx[3]; ?>" <?php if(($name==$zlxx[3] || $name==$zlxx[7] || $admin ==1 || $lcczy==1)&&($CaseStu==0)){ ?> onclick="select_ayr()"<?php }else{ ?>  readonly="readonly" <?php } ?>/></td>
                    <th class="numeric">代理人</td>
	            	<td class="numeric" colspan="2"><input style="width: 90%;" id="select_dlr" readonly="readonly" value="<?php echo $zlxx[8]; ?>" <?php if(($name==$zlxx[3] || $name==$zlxx[7] || $admin ==1 || $lcczy==1)&&($CaseStu==0)){ ?> onclick="select_dlr()" <?php }else{ ?>  readonly="readonly" <?php } ?>/></td>
	                <th class="numeric" >当前程序</td>
	                <td class="numeric">
	                <?php
	                	if($CaseST == 0){
	                		?>
	                		<select id="dqcx" name="change" <?php if(($name==$zlxx[3] || $name==$zlxx[7] || $admin ==1 || $lcczy==1)&&($CaseStu==0)){ ?> onchange="MesChange('dqcx')"<?php }else{ ?>  readonly="readonly" <?php } ?>>
	                ...
	                <!--<th>费减比</th>
	                <td><select id="FareCount" <?php if(($name==$zlxx[3] || $name==$zlxx[7] || $admin ==1 || $lcczy==1)&&($CaseStu==0)){ ?> onchange="MesChange('FareCount')"<?php }else{ ?>  readonly="readonly" <?php } ?>>
	   ...
	   <!--  案卷流程及文档    -->
          ...
          <section id="unseen">
						<input class="btn btn-success" type="button" id="remind" value="新建费用" onclick="set_remind()"/>
						<input class="btn btn-success" type="button" value="替换年费" onclick="OpenResetAnnualFee('<?php echo $ajh; ?>','zlxx')"/>
