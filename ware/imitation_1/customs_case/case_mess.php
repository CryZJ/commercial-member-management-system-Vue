	...
	<section class="panel">
		<header class="panel-heading custom-tab">
	      ...
	      <ul class="nav nav-tabs">
	        <li class="active"><a href="#about-1" data-toggle="tab"><i class="fa fa-user"></i>海关基本情况</a></li>
		      <li class="#"><a href="#about-2" data-toggle="tab"><i class="fa fa-user"></i>海关监控</a></li>
	      </ul>
      </header>
				...
				<?php
					...
					require ("../../../conn.php");
					$sql1 = "SELECT 案卷号,名称,案源人,代理人,申请号,申请日,案件备注,状态 ,备案号 FROM 海关_案件 where 案卷号='".$ajh."'";
		    ...
		    <br /><br />
				<table class="table table-bordered table-striped table-condensed" id="tab_sqr1">
	               <tr align="center">
	               		<th>案卷号</th>
	               		<td><input type="text" id="ajh" style="border-style: none;" readonly="readonly" value="<?php echo $ajh; ?>" hidden="hidden" ><?php echo $ajh; ?></td>
	               		<th>软件名称</th>
	               		<td class="numeric"><?php echo $rjmc; ?></td>
	               		<th>案源人</th>
	               		<td class="numeric"><?php echo $sqh_ayr; ?></td>
	               		<th>代理人</th>
	               		<td class="numeric"><?php echo $sqh_dlr; ?></td>
	                </tr>
	                <tr>
                		<th colspan="2">申请号</th>
                		<td align="center" class="numeric" id="sqh_rj" colspan="2"><input class="change_msg" name="申请号" style="height: 20px;" type="text" value="<?php echo $sqh_rj; ?>"/></td>
                		<th colspan="2">申请日</th>
                		<td align="center" class="numeric" colspan="2"> <input class="change_msg" name="申请日" style="height: 20px;" type="date" value="<?php echo $sqday; ?>" /></td>
	                </tr>
	            </table>
          ...
          <section id="unseen">
						...
						<?php
								$sql2 = "select * from 海关_案件 where 案卷号='".$ajh."'";
						...
						<textarea rows="5" cols="100" id="hg_bz" ><?php echo $casebz; ?></textarea>
						<br />
						<?php if($CaseST == 0){?>
	            ...
	            <?php
					   require("../../../conn.php");
						  $sql4 = "SELECT id,文件路径,操作时间,操作人,文件名  FROM 海关_文件 where 案卷号='".$ajh."' AND 状态=0";
	                ...
	                <tr align="center">
	                	...
	                	<td>
	                			<?php 
	                				$filename_arr = explode("/", $row4["文件路径"]);
													echo  $filename_arr[count($filename_arr)-1];
	                			?>
	                	</td>
	                	<td><?php echo $row4["操作时间"]; ?></td>
	                	<td><?php echo $row4["操作人"]; ?></td>
	                	<td align="center" class="numeric" colspan="4">
	                			<a class="btn btn-default" target="_blank" href="../Downloadfile.php?filename=../../<?php echo $row4["文件路径"]; ?>">
	                				下载
	                			</a>
	                			<?php if($CaseST==0){ ?>
	            ...
	            <table class="table table-bordered table-striped table-condensed" id="tab_che" style="text-align: center;">
	                ...
	                <tr hidden="hidden" >
		                	<td><select onchange="chemess(this)" id="C2" >
	                			...
	                			<?php
	                				require'../../../conn.php';
	                				$sqlSEL = "SELECT id,流程,金额,监控天数 FROM `案件流程设置` where 状态=0 and 案件类型='海关案件'";
	                ...
	                <?php
			               require("../../../conn.php");
								     $sql = "SELECT id,文件id,监控名,金额,提醒日期,截止日期,备注 FROM 海关_监控 where 案卷号='".$ajh."' and 状态=0";
		        ...
		        <thead>
		        	<th>监控名</th>
		       		<th>金额</th>
						...
						<?php
							  require("../../../conn.php");
								     $sql5 = "SELECT id,文件id,监控名,金额,提醒日期,截止日期,备注 FROM 海关_监控 where 案卷号='".$ajh."' and 状态=1";
...
<!--相关js函数-->
<script src="../../../js/imitation_1/hg_function.js"></script>
...
//				alert(data);
	...
	//修改备注后保存
	var hg_bz = document.getElementById("hg_bz");
	hg_bz.addEventListener('change',function(){
		if(confirm("是否保存备注信息？")){
			var ajhT = document.getElementById("ajh");
//			alert(zlaj_bz.value);
			$.ajax({
				type:"get",
				url:"case_mess_ajax.php",
				async:true,
				data:{
					flag:"save_hgbz",
					str_ajh:ajhT.value,
	...
	//保存修改信息
	function Save_alterdata(coltext,savevalue){
		var ajhT = document.getElementById("ajh");
		$.ajax({
			type:"get",
			url:"case_mess_ajax.php",
	...
	//监听修改信息
	$(".change_msg").change(function(){
		
		Save_alterdata($(this).attr("name"),$(this).attr("value"));
	});
