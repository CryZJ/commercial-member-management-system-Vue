				...
				<header class="panel-heading">
					...
					<?php
		    			...
		    			$CaseType = substr($ajh,7,1);
							require'../../../conn.php';
							$sql = "select 申请日 from 专案_复审等 where 案卷号='".$ajh."'";
...
//					console.log(data);
	...
	function DelRow(btn_obj){
		if(confirm("是否删除该行年费记录？")){
			$(btn_obj).parent().parent().remove();
		}
	}
	
	//创建年费表格列表
	function CreateTableRow(Tab_id_str,data){
		var tmpehtml = '';
		tmpehtml += '<tr>';
		tmpehtml += '<td>'+data["年度"]+'</td>';
//		tmpehtml += '<td><input style="width: 50%;" type="text" value="'+data["金额"]+'" /></td>';
		tmpehtml += '<td>'+data["金额"]+'</td>';
		tmpehtml += '<td>'+data["截止日期"]+'</td>';
//		tmpehtml += '<td><input style="height: 25px;" type="date" value="'+data["提醒日期"]+'" /></td>';
		tmpehtml += '<td>'+data["提醒日期"]+'</td>';
		tmpehtml += '<td>'+data["滞纳金"][0]+'</td>';
		tmpehtml += '<td>'+data["滞纳金"][1]+'</td>';
		tmpehtml += '<td>'+data["滞纳金"][2]+'</td>';
		tmpehtml += '<td>'+data["滞纳金"][3]+'</td>';
		tmpehtml += '<td>'+data["滞纳金"][4]+'</td>';
		tmpehtml += '<td><button class="btn-danger" onclick="DelRow(this)">删除</button></td>';
		tmpehtml += '</tr>';
		$("#"+Tab_id_str).append(tmpehtml);
	}
	
	//获取首年度,费减比,生成年费,申请日 直接生成年费
	function CreatFare(){
		...
		$.ajax({
			...
			success:function(data){
				for(datainfo in data){
					CreateTableRow("tab_Fare",data[datainfo]);
				}
