...
//		echo $ajh ."/" .$send_str;
		...
		//检测案件是否存在
		if(isset($ajh)){
			$sql_s = "SELECT id FROM 专利信息 WHERE 案卷号='".$ajh."'";
...
//		print_r($_FILES);
		...
		if(isset($ajh)){
			...
			if($result_s->num_rows>0){
				...
				$save_sqlpath = "filesave/".$ajh."/".$file_name;
				$sql_i = "INSERT INTO 案卷流程及文档(案卷号,处理人,时间,文件路径,流程) VALUES(";
				$sql_i .= "'".$ajh."','".$name."','".date("Y-m-d H:i:s")."','".$save_sqlpath."','新建流程')";
				if($conn->query($sql_i)){
					$sql_s = "SELECT id FROM 案卷流程及文档  WHERE 案卷号='".$ajh."' AND 流程='新建流程'  ORDER BY id DESC LIMIT 1";
					$result_s = $conn->query($sql_s);
					if($result_s->num_rows>0){
						...
						if($result_s2->num_rows>0){
							...
							$ret_mag = "“专案_监控”没有相应的监控";
						}
					}else{
						$ret_mag = "“案卷流程及文档”中没有案卷号为".$ajh."的监控";
					}
				}else{
					$ret_mag = "案卷流程及文档保存失败";
				}
