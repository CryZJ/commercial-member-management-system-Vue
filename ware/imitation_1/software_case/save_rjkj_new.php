...
//		echo $ajh ."/" .$send_str;
		...
		if(isset($ajh)){
			...
			if($result_s->num_rows>0){
				...
				$sql_i = "INSERT INTO 软件_监控(案卷号,监控名,金额,提醒日期,截止日期,备注) VALUES(";
...
//		print_r($_FILES);
		...
		if(isset($ajh)){
			...
			if($result_s->num_rows>0){
				...
				$save_sqlpath = "filesave_rj/".$ajh."/".$file_name;
				$sql_i = "INSERT INTO 软件_文件(案卷号,处理人,时间,路径) VALUES(";
				$sql_i .= "'".$ajh."','".$name."','".date("Y-m-d H:i:s")."','".$save_sqlpath."')";
				if($conn->query($sql_i)){
					$sql_s = "SELECT id FROM 软件_文件  WHERE 案卷号='".$ajh."' ORDER BY id DESC LIMIT 1";
					$result_s = $conn->query($sql_s);
					if($result_s->num_rows>0){
						while($row_s = $result_s->fetch_assoc()){
							$file_id = $row_s['id'];
						}
						$sql_s2 = "SELECT id FROM 软件_监控  WHERE 案卷号='".$ajh."' ORDER BY id DESC LIMIT 1";
						$result_s2 = $conn->query($sql_s2);
						if($result_s2->num_rows>0){
							while($row_s2 = $result_s2->fetch_assoc()){
								$monitor_id = $row_s2['id'];
							}
							$sql_u = "UPDATE 软件_监控  SET 文件id = '".$file_id."' WHERE id='".$monitor_id."'";
							if($conn->query($sql_u)){
								$ret_mag = "保存成功";
							}else{
								$ret_mag = "保存失败";
							...
							$ret_mag = "“软件_监控”没有相应的监控";
						}
					}else{
						$ret_mag = "“软件_信息”中没有案卷号为".$ajh."的监控";
					}
				}else{
					$ret_mag = "软件_文件保存失败";
				}
	...
	if($flag == 'ChangeSitu'){
		$MesId = $_GET['id'];
		$sql_C = "update 软件_监控  set 状态=1 where id = '".$MesId."' ";
		if($conn->query($sql_C)){
			$ret_mag = "操作成功";
		}else{
			$ret_mag = "操作失败,请联系管理员";
	...
	if($flag == 'DJId'){
		$mes = $_GET['mes'];
		$ajh = $_GET['ajh'];
		$Save_DJ = "update `软件_信息` set `登记号`='".$mes."' where 案卷号='".$ajh."'";
