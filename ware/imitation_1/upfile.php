...
<body>
	<div align="center">
		<div id="my_form">
			...
			<input type="text" id="clr" value="<?php echo $name; ?>" hidden="hidden" />
			<?php
				require("../../conn.php");
				$sql = "SELECT id FROM 案卷流程及文档 WHERE 案卷号='".$_GET['ajh']."' AND 流程='待提交' AND 删除状态='0'  AND (SELECT SUBSTRING_INDEX(文件路径,'.',-1))='zip'";
				$result = $conn->query($sql);
				if($result->num_rows>0){
			?>
					<input type="text" id="lc" value="上传文件" hidden="hidden" />
			<?php
				}else{
			?>
					<strong>检测到该案件的（zip）申请文件被删除或没上传,请上传后才能上传其他文件</strong>
					<input type="text" id="lc" value="待提交" hidden="hidden" />
			<?php		
				}	
			?>
			<table>
...
//			tmp_td_2.appendChild(tmp_input);
	...
	document.getElementById("uploading").addEventListener("click",function(){
		...
		var clr_v = document.getElementById("clr").value;
		var lc_v = document.getElementById("lc").value;
		fd_file.append("flag","uploadfile_za");
		fd_file.append("ajh",ajh_v);
		fd_file.append("clr",clr_v);
		fd_file.append("lc",lc_v);
		if(file_num > 1){
			$.ajax({
				type:"POST",
				url:"upfile_ajax.php",
