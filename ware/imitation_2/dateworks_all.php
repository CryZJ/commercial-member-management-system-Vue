			    ...
			    <section class="panel">
			      <header class="panel-heading custom-tab">
				      <ul class="nav nav-tabs">
				        ...
				        <li class="active"><a href="#about-1">全部日程</a></li>
				        <input id="NORS" hidden="hidden" value="<?php echo $normal; ?>" /><!--勿删dateworks_finish.php-->
				      </ul>
			      </header>
...
//                      		echo $order[2];
                    ...
                    <span class="dynamic-table" hidden="hidden" >1/asc/计划时间【正】</span>
	                        ...
	                        <tr>
	                            ...
	                            <th style="width: 12%;">计划时间</th>
	                            <th style="width: 50%;">计划安排</th>
                        ...
                        <tbody>
                        	<?php
                        		require'../../conn.php';
                        		if($admin == "1"){
                    					$sql = "select id,用户id,事件名,事件时间,状态,备注  from 日程设置 where 删除状态=0 order by id desc";
                    				}else{
                    					$sql = "select id,用户id,事件名,事件时间,状态,备注  from 日程设置 where 删除状态=0 and 用户id='".$userid."' order by id desc";
                    				}
				                		...
				                		if($result->num_rows>0){
				                            ...
				                            <td class="numeric"><?php if($CSt){echo "已完成";}else{echo "未完成";} ?></td>
				                            <td class="numeric"><?php echo $yhm; ?></td>
				                        <?php 
				                        	$sql2 = "SELECT id FROM 日程文件 WHERE 日程id='".$CId."'";
