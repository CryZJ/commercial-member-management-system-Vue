...
<section>
        ...
        <section class="panel">
            <header class="panel-heading">
            	缴费通知信息确认
            	<input hidden="hidden" type="text" value="<?php $id = $_REQUEST['mas']; echo $id; ?>" id="mess" />
            ...
            <div class="panel-body" style="width:98%;  overflow:auto; solid #000000;">
            	<!--<span><strong>选择需导出案件</strong></span>-->
            	<input type="button" class="btn btn-primary" id="" value="选择案卷号" onclick="select_ajh()" />
            	<br /><br />
            	<table class="display table table-bordered table-striped" id="ajhShow">
                  	<tr>
                  		<th style="width: 150px;" >案卷号</th>
                  		<th></th>
                  	</tr>
            	</table>
            	<!--申请人-->
            	...
            	<br />
				<a href="#" class="btn btn-primary" onclick="op_linkman()">选择客户联系人 </a>
				<br />
				<br />
				<input id="sqr_id" hidden="hidden" />
          	...
          	<div>
	          	<center>
	              	<input class="btn btn-primary" type="button" id="btn2" value="导出通知" onclick="exportWord()" />
	          	</center>
          	</div>
         </div>
       	</section>
...
<script type="text/javascript">
	...
	//选择联系人
	function op_linkman(){
		var id = document.getElementById("sqr_id").value;
...
//			alert(NumAll);
	...
	function select_ajh(){
		...
		var left = (scr_width-bro_width)/2;
		var specs = "height="+bro_height+",width="+bro_width+",top="+top+",left="+left;
		var winobj = window.open("../../select_ajhC.php","_blank",specs);
		var loop = setInterval(function(){
			if(winobj.closed){
				clearInterval(loop);
				if(typeof(Storage)!=="undefined"){
					if(localStorage.return_data){
						return_data = localStorage.return_data;
						document.getElementById("ajhShow").rows[0].cells[1].innerHTML = return_data;
						//获取申请人名字
						GetSqrName(return_data);
						ShowFare(return_data);
	...
	function GetSqrName(ajh){
		$.ajax({
			...
			success:function(data){
				...
				document.getElementById("sqr_name").value = ArrData[1];
				document.getElementById("sqr_id").value = ArrData[0];
//				alert(data);
//				console.log(data);
			}
		});
