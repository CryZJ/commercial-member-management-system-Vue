...
<body class="sticky-header">
        ...
        <div class="adv-table">
        	...
        	<table class="display table table-bordered table-striped" >
        			<tr>
        				<th colspan="2">操作员</th>
        				<th colspan="7"><input type="text" id="cpeo" value="<?php echo $name; ?>" readonly="readonly" hidden="hidden"  /><?php echo $name; ?></th>
        				
        				<th hidden="hidden" ><input type="text" id="fid" value="<?php echo $id; ?>" hidden="hidden" /></th>
        			</tr>
        			<tr>
...
//		        		echo '<script>alert("'.$len.'")</script>';
		        		require'../../conn.php';
		        		for($i=0;$i<$len;$i++){
		        			$sql = "SELECT id,c.案卷号,专利名称,申请人,申请号,申请日,年度,金额,应缴日期,剩余天数,通知书生成日期,缴费时间,收据上传日期,d.滞纳金  FROM ((SELECT 案卷号,专利名称,申请人,申请号,申请日 FROM 专利信息 ) UNION (SELECT 案卷号,专利名称,申请人,申请号,申请日 FROM 专案_年费 )) as c ,`专案_年费记录` d where d.id='".$fid[$i]."' and c.`案卷号`=d.`案卷号`";
        	...
        	<form action="yearcost_check_save.php" enctype="multipart/form-data" method="post" >
        		<input type="text" id="fid" name="fid" value="<?php echo $id; ?>" hidden="hidden" />
        		<input type="text" id="cpeo" name="cpeo" value="<?php echo $name; ?>" hidden="hidden" />
        		<table>
	        		<tr style="width: 500px;" >
	        			<th width="20%" >年费收据编号：</th>
