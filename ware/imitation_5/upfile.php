...
//			tmp_html +='<td><input class="miaoshu" type="text" placeholder="请填写文件描述" tabindex="'+file_num+'" /><button onclick="del_addfile(\''+tmp_id+'\')">取消</button></td>';
//			tmp_html += '</tr>';
...
//			var fd = new FormData();
			...
			$("#file_list input[class='miaoshu']").each(function(i){
				if($(this).attr("value") != ""){
					des_arr[i] = $(this).attr("value");
				}else{
					des_arr[i] = "无";
				}
			});
