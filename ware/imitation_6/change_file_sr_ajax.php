...
//		$des_str = $_POST['des'];
		$ret_data = "";
//		$sql_s = "SELECT 收入id,文件路径 FROM 收入记录文件 WHERE id = '".$id."'";
		$sql_s = "SELECT 收入id,文件路径 FROM ".$earn_file_record." WHERE id = '".$id."'";
...
//				$sql = "UPDATE 收入记录文件 SET 文件路径='".$path_sql."',上传时间='".date("Y-m-d H:i:s")."',上传人='".$name."' WHERE id = '".$id."'";
				$sql = "UPDATE ".$earn_file_record." SET 文件路径='".$path_sql."',上传时间='".date("Y-m-d H:i:s")."',上传人='".$name."' WHERE id = '".$id."'";
				if($conn->query($sql)){
